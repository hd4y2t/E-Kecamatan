...
class Pemerintahan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // KASIH PEMERINTAHAN
        $data['skn'] = $this->pegawai->skn();
        $data['countskn'] = $this->pegawai->Countskn();

        $data['skdp'] = $this->pegawai->skdp();
        $data['countskdp'] = $this->pegawai->Countskdp();

        $data['sku'] = $this->pegawai->sku();
        $data['countsku'] = $this->pegawai->Countsku();

        $data['skd'] = $this->pegawai->skd();
        $data['countskd'] = $this->pegawai->Countskd();

        $data['skos'] = $this->pegawai->skos();
        $data['countskos'] = $this->pegawai->Countskos();

        $data['spc'] = $this->pegawai->spc();
        $data['countspc'] = $this->pegawai->Countspc();
    ...
    public function penduduk()
    {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/navbar', $data);
        $this->load->view('pemerintahan/penduduk', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function profile()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/navbar', $data);
            $this->load->view('pemerintahan/profile', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            if ($this->upload->do_upload('foto_camat,foto_sekcam')) {
                ...
                $this->session->set_flashdata('success', 'Berhasil Ditambahkan!');
                redirect(base_url("pemerintahan/profile"));
            }
        }
    }
}
